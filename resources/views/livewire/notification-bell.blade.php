    ...
    <div style="position:absolute;top:calc(100% + 8px);right:0;width:320px;max-height:420px;overflow-y:auto;background:#141416;border:1px solid rgba(255,255,255,0.09);border-radius:10px;box-shadow:0 12px 32px rgba(0,0,0,0.45);z-index:50;">
        ...
        @else
            <div>
                ...
                <a href="{{ $notification->data['url'] ?? '#' }}" wire:click="markAsRead('{{ $notification->id }}')" style="display:block;padding:0.7rem 1rem;border-bottom:1px solid rgba(255,255,255,0.05);cursor:pointer;text-decoration:none;{{ $notification->read_at ? 'opacity:0.55;' : '' }}">
        ...
        <div style="padding:0.6rem 1rem;border-top:1px solid rgba(255,255,255,0.06);text-align:center;">
            <a href="{{ route('notifications.index') }}" style="font-size:11px;color:#22c55e;text-decoration:none;font-weight:600;">View all</a>
        </div>
    </div>
    @endif
</div>
